...
Claude API와 대화하며 캐릭터를 디자인하고, Stable Diffusion으로 픽셀 애니메이션을 자동 생성합니다.
...
SD_API_URL=http://127.0.0.1:7860    # SD WebUI 주소
```

SD WebUI는 반드시 `--api` 플래그로 실행해야 합니다:
...
현재 테스트는 외부 서비스(SD WebUI, Claude API) 없이 실행 가능합니다.
...
print(f'후처리 완료: 고유 색상 수 확인')
"
```

> `rembg`가 미설치이거나 GPU 이슈 시 `remove_bg=False`로 배경 제거를 건너뛸 수 있습니다.
...
for i in range(4):
    ...
    arr[30:90, 30+i*5:90+i*5, :3] = 200  # 이동하는 사각형
...
#### 5. E2E 테스트 (모든 외부 서비스 필요)

```bash
# SD WebUI 실행 + ANTHROPIC_API_KEY 설정 후
python chat.py
```
